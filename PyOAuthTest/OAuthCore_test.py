...
from PyOAuthClient import OAuthException
...
from PyOAuthClient.Models.OAuthModels import IdTokenClaims, AccessTokenClaims, OAuthProviderMetadata
...
class TestOidcCoreStatic:
    ...
    def test_decodeIdToken(self):
        ...
        assert OAuthCore.decodeIdToken(idToken) == IdTokenClaims(
            iss="https://logto.app",
            aud="foo",
            exp=1616446400,
            iat=1616446300,
    ...
    def test_decodeAccessTokenWithResourceAndClientId(self):
        ...
        assert OAuthCore.decodeAccessToken(accessToken) == AccessTokenClaims(
            iss="https://logto.app",
...
class TestOidcCore:
    @pytest.fixture
    def oidcCore(self, metadata: OAuthProviderMetadata) -> OAuthCore:
        return OAuthCore(metadata)
    ...
    async def test_getProviderMetadata(
        ...
        result = await OAuthCore.getProviderMetadata(discovery_url)
    ...
    async def test_fetchTokenByCode_failure(
        ...
        with pytest.raises(OAuthException, match="error"):
